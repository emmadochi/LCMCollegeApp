...
export function initDashboardChat() {
    ...
    const chatCourseSelect = document.getElementById('chatCourseSelect');

    if (!chatFabBtn || !closeChatBtn || !chatPanel || !chatForm || !chatCourseSelect) return;

    // Load available enrolled courses into select dropdown
    loadCoursesDropdown();
    ...
    chatFabBtn.addEventListener('click', () => {
        ...
        if (isChatOpen) {
            ...
            document.getElementById('chatBubbleNotification').classList.add('hidden'); // Hide bubble
    ...
    // Change course select
    chatCourseSelect.addEventListener('change', () => {
        currentCourseId = chatCourseSelect.value;
        ...
        if (currentCourseId) {
            ...
            lastMessageId = null; // reset message history tracking on selection change
            pollInterval = setInterval(loadMessages, 3000);
        } else {
            chatInput.setAttribute('disabled', 'true');
            if (sendBtn) sendBtn.setAttribute('disabled', 'true');
            document.getElementById('chatMessagesContainer').innerHTML = `
                <div class="text-center py-8 text-gray-400 text-xs" id="chatSelectorPrompt">
                    <i class="fa-regular fa-comment-dots text-2xl mb-2 text-gray-300 block"></i>
                    Please choose a course above to load chat logs.
                </div>
            `;
            if (pollInterval) {
                clearInterval(pollInterval);
    ...
    chatForm.addEventListener('submit', async (e) => {
        ...
        if (!msgText || !currentCourseId) return;
...
async function loadCoursesDropdown() {
    try {
        ...
        if (response.ok) {
            ...
            // Let's retrieve enrolled list from storage
            const user = JSON.parse(localStorage.getItem('user') || '{}');
            const enrolledIds = user.enrolledCourses || [];
            const enrolledCourses = coursesList.filter(c => enrolledIds.includes(c.id));

            const select = document.getElementById('chatCourseSelect');
            if (select) {
                const currentVal = select.value;
                select.innerHTML = '<option value="">-- Choose Course / Lecturer --</option>';
        ...
        if (response.ok) {
            ...
            if (badge && data.unread_count > 0) {
                ...
                if (data.unread_count > lastUnreadCount && bubble && bubbleText) {
                    ...
                    // Try to determine which course has a new message
                    // Let's fetch all messages for enrolled courses
                    // To do this simply, we fetch latest messages for any course having unread
                    // Or let's fetch messages of the first course we find having messages
                    // For the dashboard preview, let's query the unread endpoint
                    // Let's fetch all active chats using messages.php or load active chat logs
                    // Let's just fetch the messages of any enrolled course to find the latest lecturer message
                    const user = JSON.parse(localStorage.getItem('user') || '{}');
                    const enrolledIds = user.enrolledCourses || [];
                    
                    for (const cid of enrolledIds) {
                        try {
                            const msgRes = await fetch(`../api/chat/messages.php?course_id=${cid}`, {
                                method: 'GET',
                                headers: {
                                    'Content-Type': 'application/json',
                                    ...getAuthHeader()
                            ...
                            if (msgRes.ok) {
                                ...
                                if (lastLecturerMsg && !lastLecturerMsg.is_read) {
                                    ...
                                    currentCourseId = cid; // Presets course when they open it
                                    const select = document.getElementById('chatCourseSelect');
                                    if (select) {
                                        select.value = cid;
                                        // Enable form
                            ...
                            console.error("Error loading dashboard preview cid:", err);
                        }
                    }
                }
            } else {
